...
Follower::Follower(SDL_Renderer* ren, SDL_Texture* tex, SDL_Rect dest, SDL_Rect* playerDest, float speed, float damage, int radius, int playerRadius) :
	Object(dest, playerDest), Entity(), AI(playerDest, &m_dest), m_speed(speed), m_damage(damage), m_wonderRadius(radius), m_searchPlayerRadius(playerRadius) {
	m_ren = ren;
	m_tex = tex;
...
void Follower::Update() {
	if (m_isAlive) {
		...
		if (m_isPlayerInRadius) {
			m_destination = AI::Follow(m_speed, Vec2(m_enemyDest->x, m_enemyDest->y));
